...
###Q2 Using `skim()` display the summary of variables,Think about the task to predict a family income based on their menu: what variables may be useful? Are all of them correct type? Think about the task to predict a community type or US_region based on their menu: what variables may be useful? Are all of them correct type? 
...
#According to the chart we can see that roasted is the most popular method for cooking all main_dish. People can use various of methods to cook Turkey. And only use one method to cook Turducken which is roasted. 
...
###Q10 Use the following code to create a new data set.
...
#Select id first. The rest of them start with "side, pie and dessert". Then, select side15, pie13, dessert12 and remove them. Gather type, value and id, after that rank id with descending order. Filter the invalid value and add a new column named "type". And remove type's("\\d+") which means match 2 or more digits. Create new data set variable named "side_pie_dessert". 

```

###Q11-12 Intall package `widyr` and use `pairwise_cor()` function https://www.rdocumentation.org/packages/widyr/versions/0.1.3/topics/pairwise_cor.Write 2-3 sentences with your explanation of what it does. 
...
#Explain the correlation between different types of items.
...
#From the data set we can see, when I use 'celebrate' column to predict a family_income, it shows that the estimate of error rate is 82.05%. That is to say, this result is inaccurate forecast. 
...
#According to the second data set, when I use 'community', 'age', 'living region' to predict family income which has an estimate of error rate 82.71%. This result can be considered inaccurate as well. 
